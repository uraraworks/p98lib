...
/* 640x400 16色モードを開始する。書き換える割り込みベクタ(INT 23h)は
 * 退避し、Ctrl+C による異常終了時に画面がグラフィックモードのまま
 * 固まらないよう INT 23h を無害化する。
 *
 * ---- パレットは「読み出して退避」ではなく既定値へ明示設定(2026-09後半) ----
 * 以前はp98_init()が現在のパレット16色をポートから読み出して退避し、
 * p98_quit()で書き戻していたが、**その読み出しがnp2kai上では実際の値を
 * 返さない**ことが実測で分かった(同じFreeDOSセッション内でp98libの
 * プログラムを2回連続で実行すると、2回目のp98_quit()が画面全体を一色に
 * 潰してしまう不具合として顕在化した。docs/design.md・docs/verify-log.md
 * 参照)。そのためp98_init()は読み出しに頼らず、実測したFreeDOS起動直後の
 * 既定パレット16色をそのまま書き込む。何度呼び出しても常に同じ状態から
 * 始まる。
 *
...
/* p98_init() で変更した状態(画面モード・INT 23h ベクタ)を元へ戻し、
 * パレットは p98_init() と同じ既定値16色へ書き戻す(呼び出し前の実際の
 * パレットを読み出して復元するわけではない。上の p98_init() のコメント
 * 参照)。p98_init() を呼んでいない状態で呼んではいけない。
 *
