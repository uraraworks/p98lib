/* 故障注入: tools/kya_convert.mjs generate --broken-swap-rg で生成した
 * (R/Gプレーンを入れ替えた)版を使う。probe_walk2_assets.cと全く同じ座標へ
 * 同じ絵を描くが、色が化けているはずなので、正しい期待値との比較検査が
 * FAILすることを確認するためのプローブ(tools/verify.mjs参照)。
 */
#include "p98.h"
#include "kya_assets_broken_rg.h"
