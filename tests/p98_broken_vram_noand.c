/*
 * ★故障注入版(tests/p98_broken_vram_noand.c)★
 * src/p98.c のコピーに対し、p98_draw_sprite_vram()の2パス転送のうち
 * パス1(反転マスクをANDして、これから描くドットぶんだけ背景を0にする
 * 処理)だけを丸ごとスキップするよう壊してある(`if (!vs->opaque)` を
 * `if (0 && !vs->opaque)` に変更しただけ。該当箇所を検索する場合は
 * "故障注入: 条件に0を&&して" を探すこと)。パス2(絵をORで重ねる転送)は
 * 手を付けていない。
 * 結果として、透明ドットのある位置で背景が0にならないまま絵のビットが
 * そのままORされ、CPU経路(p98_draw_sprite)の結果とずれるはず。
 * tools/verify.mjsの陰性対照(等価性検査がFAILを検出できることの確認)
 * 専用。通常のビルド・配布物には含めない。
 *
 * p98.c - p98.h の実装。
 *
...
void p98_draw_sprite_vram(const p98_vram_sprite_t *vs, int x, int y) {
    ...
    if (0 && !vs->opaque) { /* ★故障注入: 条件に0を&&してパス1(AND転送)を丸ごとスキップさせている。
                              * 透明ドットの位置で背景が0にならないまま、パス2のOR転送が
                              * かぶさるので、絵のビットがそのまま背景にORされてしまう。 */
