 ...
 * ユーザー本人の著作物SHUTING.ASMの実例(AH=0x12でカーソルを消す)とも一致)。
...
int p98_init(void) {
    ...
    /* テキスト画面を消し、カーソルを隠す(既定。docs/design.mdの
     * 「テキスト画面とカーソルの後始末」節参照)。FreeDOS起動時のメッセージ等が
     * グラフィック画面と重なって見えてしまう問題への対策。テキスト画面の
     * “中身”は退避しない(戻せない。p98_quit()側のコメント・docs/design.md参照)。
     * カーソルの表示/非表示は必ずp98_quit()で表示に戻す。 */
