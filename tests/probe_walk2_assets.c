/* samples/mag_assets.h (MAG変換ツールの出力。ORIGINAL/KYARA-03.MAG由来)が、
 * VRAM上に期待どおりのドット列で出ることを確認するためのプローブ。
 * バイト境界に揃った座標へタイル2種+4方向の代表フレームを1枚ずつ描き、静止する。
 * 期待値はtools/verify.mjs側でtools/mag_convert.mjsのbuildAssetSetFromMag()を
