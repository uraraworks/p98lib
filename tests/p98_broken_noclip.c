/*
 * ★故障注入版(tests/p98_broken_noclip.c)★
 * src/p98.c のコピーに対し p98_fill_rect() のクリップ処理だけを取り除いてある。
...
void p98_fill_rect(int x, int y, int w, int h, int color) {
    ...
    /* 故障注入(tests/p98_broken_noclip.c): わざとクリップを外している。
     * docs/verify-log.md の陰性対照用で、通常ビルドでは使わない。 */
    if (w <= 0 || h <= 0) return;
    x0 = x;
    y0 = y;
    x1 = x + w;
