/* p98_copy_bgpage_to_screen()のA/B速度比較(Bの側)。
 * tests/probe_bgcopy_bench_2x.c(A=draw_tiled_background()を背景・画面へ
 * それぞれ1回ずつ計2回)と「タイル・タイル選択規則・座標順・REPEAT回数」を
 * 完全に揃え、こちらは背景ページへ1回だけ敷いてからp98_copy_bgpage_to_screen()
 * でまるごとコピーする形にする。
 */
#include "p98.h"
