 ...
 * 同じファイルを切り出している(samples/mag_assets.hはtools/mag_convert.mjs
 * による自動生成のため手編集しないこと)。
 *
 *   - キャラ: 32x32、UP/DOWN/LEFT/RIGHT 各2フレーム。
 *   - 地面: 16x16タイル2種(草・レンガ)。画面全体へ敷き詰める。
 *
 * 描画方式: p98_init_bgpage() で背景ページへタイルを敷き、以後は差分復帰で
 * キャラだけを動かす。画面ページへは p98_copy_bgpage_to_screen() で
 * まるごとコピーする。タイル・キャラとも p98_vram_upload() でVRAMの
 * 置き場へ常駐させ、p98_draw_sprite_vram()系で描く。置き場が768B/プレーン
 * しかないため、キャラは「いま向いている方向の2コマ」だけを常駐させ、
 * 向きが変わったときだけ置き直す(draw_character()参照)。
 * p98_vram_upload()が失敗した場合は、その系統だけ従来のCPU経路
 * (p98_draw_sprite()/p98_draw_sprite_diff())へフォールバックする。
 *
 * 操作: 方向キーを押している間歩き続ける。4フレームに1回だけ移動判定する。
 * ESCで終了する。
 *
